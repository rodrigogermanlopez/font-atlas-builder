/*
 ...
 Modifications: Rodrigo Lopez
 New AS3 Array API added, removed the EventDispatcher
...
public class MaxRectsBinPack {
	...
	protected var tmpRects2:Vector.<Rectangle>;
...
//				freeRectangles.splice(i, 1);
	...
	public function insertBulkWorker( event:Event ):void {
		var t:int = getTimer();
//		trace("temp rects:", tmpRects.length)
		var bestScore:Score = new Score();
		var tmpScore:Score = new Score();
		while ( tmpRects.length > 0 ) {
			if ( getTimer() - t < timeLimit ) {
				bestScore.setEmpty();
				var bestRectIndex:int = -1;
				var bestRectIndex2:int = -1;
				var bestNode:Rectangle = rectFactory();
				for ( var i:uint = 0; i < tmpRects.length; ++i ) {
//					var score:Score = new Score();
					tmpScore.setEmpty();
					var newNode:Rectangle = scoreRect( tmpRects[i].width, tmpRects[i].height, tmpMethod, tmpScore );
					if ( tmpScore.score1 < bestScore.score1 || (tmpScore.score1 == bestScore.score1 && tmpScore.score2 < bestScore.score2) ) {
						bestScore.score1 = tmpScore.score1;
						bestScore.score2 = tmpScore.score2;
						bestNode = newNode;
						bestRectIndex = i;
						bestRectIndex2 = tmpRects2.indexOf( tmpRects[i] );
					}
				}
				if ( bestRectIndex == -1 ) {
				...
				// this sucks, but we need pointers.
				indices.push( bestRectIndex2 );
				placeRect( bestNode );
//				tmpRects.splice(bestRectIndex, 1);
				tmpRects.removeAt( bestRectIndex );
			} else {
				++frameCounter;
...
//		trace("num frames to complete:" , frameCounter) ;
//		onStatus.dispatch( "complete" );
...
//				freeRectangles.splice(i, 1);
	...
	protected function findPositionForNewNodeBestShortSideFit( width:Number, height:Number,
															   score:Score = null ):Rectangle {
		if ( score == null ) {
			score = new Score();
		}
		score.score1 = Number.MAX_VALUE;
	...
	protected function findPositionForNewNodeBestLongSideFit( width:Number, height:Number,
															  score:Score = null ):Rectangle {
		if ( score == null ) {
			score = new Score();
		}
	...
	[Inline]
	private final function pruneFreeList():void {
...
class Score {
	...
	public function setEmpty():void {
		score1 = score2 = Number.MAX_VALUE;
	}
}